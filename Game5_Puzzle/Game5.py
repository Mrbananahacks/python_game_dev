...
import random
import pygame 
from cfg import BACKGROUNDCOLOR


def isGameOver(board, size):
    assert isinstance(size, int)
    num_cells = size*size
    for i in range(num_cells-1): # for the number of cells do this
        if board [i] != i: return False # if the cell we are at doesnt equal the correct cell say False
    return True
